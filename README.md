...
**Post-weighting balance check.** `covariate_balance_weighted.csv` and `figures/covariate_balance_before_after.png` compare each covariate's SMD before and after weighting. All 34 covariates that exceeded |SMD| > 0.1 before weighting fall under that threshold after weighting: 0 of 63 remain imbalanced. The adjustment worked, the weighted RHC and no-RHC groups are now comparable on every covariate checked.
...
**Setup.** Three scikit-learn classifiers, logistic regression (baseline), random forest, and gradient boosting, predict `death_d30` from the same 63 admission-characteristic covariates used in Phases 2-3 (the RHC treatment indicator itself is deliberately excluded, keeping this a risk score based on admission data, not a treatment-effect model). Data is split 80/20 (stratified) into train and test sets; models are compared via stratified 5-fold cross-validation on the training set, then evaluated once on the untouched test set.

**Model comparison.** All three models perform similarly (`cv_model_comparison.csv`): cross-validated ROC-AUC of 0.758 (logistic regression), 0.763 (random forest), and 0.767 (gradient boosting). On the held-out test set (`test_set_metrics.csv`, `figures/roc_curves.png`), ROC-AUC lands at 0.767, 0.782, and 0.784 respectively, with gradient boosting and random forest offering a modest edge over the linear baseline, but not a dramatic one. That the interpretable baseline stays competitive with the ensembles is itself worth noting: it suggests the relationship between admission characteristics and 30-day mortality is largely linear/additive rather than driven by complex interactions.

**Feature importance.** Permutation importance (`feature_importance.csv`, `figures/feature_importance.png`), the drop in test-set ROC-AUC when a feature is randomly shuffled, is used across all three models so the comparison isn't skewed by any one model's built-in importance metric. `surv2md1` (the SUPPORT model's own 2-month survival estimate) dominates in every model, unsurprising since it's already a mortality-risk estimate itself. Beyond that, `no_resus_Yes` (DNR status), `bili1` (bilirubin), `aps` (APACHE severity), and `category_Coma` consistently rank near the top across all three models, a plain-language read: liver function, overall physiologic severity, neurological status, and code status are the strongest drivers of 30-day mortality risk at admission.
